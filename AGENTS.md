...
- 教材、考题、答案、面试题和点评中的关键结论，必须能追溯到明确来源，例如论文、官方文档、权威技术博客、厂商文档或近期面经。
- 不允许只在文末堆一个泛泛的来源列表。正文中的核心概念、架构建议、数据、最佳实践、风险判断和题目来源，必须用 `[来源: 名称]` 或等价方式标注出处。
- 每个文件末尾必须保留 `## 来源`，并列出完整 URL；来源名称要和正文标注可对应。
- 如果某段内容是基于多个来源综合推理得出，必须写明“综合自哪些来源”；如果没有找到可靠外部来源，必须明确标注“未找到外部验证，仅为推理”。
- 面向大厂/AI 独角兽的考题和面试题，必须标注题型依据来自近期面经、官方岗位要求、产品/平台文档或公开技术实践中的哪一类来源。
...
| 阶段 | 验证要求 |
|------|----------|
| **备课** | 每个知识点至少查 2 个独立来源；教案正文中的关键结论逐条标注引用，文末列完整来源 |
...
1. **先搜索**：用 WebSearch 搜索关键词的权威资料
   ...
   - 记录至少 3 个可靠来源
   - 建立“内容点 → 来源”的映射，确保后续正文引用能追溯到具体来源

2. **再生成**：基于验证后的资料，在 `materials/YYYY-MM-DD-<slug>/` 下生成以下文件

3. **最后标注**：每个文件正文标注关键结论来源，末尾附上完整来源列表
