...
每个材料文件必须有稳定结构，不能只自由发挥。允许根据主题增加小节，但不得缺少以下必备块。
...
3. **exam-answers.md**
   ...
   - 开头写明答案依据，标注来源。
   - 每题使用与 `exam.md` 对应的 `## N. <题目名>（难度）`。
   - 每题必须包含：标准答案、解析、常见错误或扣分点、主要依据。
   - 末尾 `## 来源`。
...
5. **interview-answers.md**
   - `# <keyword> — Interview Reference Answers`
   - 开头写明评分标准依据，标注来源。
   - 每题使用与 `interview.md` 对应的 `## N. <题目名>`。
   - 每题必须包含 `题目原文：`，原样写入面试题正文。
   - 每题必须包含 `追问：`，原样写入对应追问。
   - 每题必须包含 `满分回答要点：`、`示范回答：`、`常见错误：`、`主要依据：`。
   - `示范回答：` 必须是普通段落或列表；不要放在代码块里，除非题目明确要求写代码、命令或结构化配置。
   - 末尾 `## 来源`。

6. **interview-transcript.md**
   - `# <keyword> — Interview Transcript`
   - 写明日期、主题、状态。
   - 每题必须包含：问题、用户回答摘要、点评、追问过程。
   - 文末包含总体评价：优点、待加强、结论。
   - 末尾 `## 来源`。
